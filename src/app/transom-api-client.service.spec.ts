...
describe("TransomApiClientService", () => {
  ...
  it("should make contact request submissions", done => {
    const obs = apiClient.postRequest({ requestData: { personalInfo: { foo: "bar" } }, attachments: [] });
    ...
    const req = httpTestingController.expectOne('/api/v1/fx/submitContactRequest');
  ...
  it("can handle an error when making a request", done => {
    const obs = apiClient.postRequest({ requestData: { personalInfo: { foo: "bar" } }, attachments: [] });
    expect(obs).toBeTruthy();
    obs.subscribe(value => {
